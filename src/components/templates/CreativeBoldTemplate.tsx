...
export function CreativeBoldTemplate({ data, palette, fonts }: TemplateProps) {
  ...
  return (
    ...
    >
      ...
      <header className="relative px-[18mm] pt-[18mm] pb-6 pr-[52mm]">
        ...
        <AvatarPhoto
          ...
          className="absolute top-[18mm] right-[18mm]"
        />
      </header>
      ...
      <div className="px-[18mm] pb-[18mm] grid grid-cols-[1fr_2fr] gap-8">
        ...
        <aside className="space-y-5">
          <Block title={sectionTitleLabel("Contact")} palette={palette}>
          ...
          {data.skills.length > 0 && <Block title={sectionTitleLabel("Skills")} palette={palette}><SkillsTags data={data} palette={palette} /></Block>}
          {data.languages.length > 0 && <Block title={sectionTitleLabel("Languages")} palette={palette}><LanguagesList data={data} palette={palette} /></Block>}
          {data.certifications.length > 0 && <Block title={sectionTitleLabel("Certifications")} palette={palette}><CertificationsList data={data} palette={palette} /></Block>}
        </aside>
            ...
            switch (id) {
              case "summary":
                return personal.summary ? (
                  <Block key={id} title={sectionTitleLabel("About")} palette={palette}>
                    <p className="text-[11pt] leading-[1.6] italic" style={{ color: palette.secondary }}>"{personal.summary}"</p>
                  </Block>
                ) : null;
              case "experience":
                return data.experience.length ? <Block key={id} title={sectionTitleLabel("Experience")} palette={palette}><ExperienceList data={data} palette={palette} /></Block> : null;
              case "education":
                return data.education.length ? <Block key={id} title={sectionTitleLabel("Education")} palette={palette}><EducationList data={data} palette={palette} /></Block> : null;
              case "projects":
                return data.projects.length ? <Block key={id} title={sectionTitleLabel("Projects")} palette={palette}><ProjectsList data={data} palette={palette} /></Block> : null;
              default:
                return null;
            }
          })}
